...
class AuthSignupHome(Home):
    ...
    def do_signup(self, qcontext):
        ...
        values = {key: qcontext.get(key) for key in (
            'login', 'name', 'password', 'national_id')}
        values.update({'national_id': (qcontext.get('national_id'))})
        print(values)
        ...
        if not values.get('national_id'):
            raise UserError(_("national_id must be entered"))
        if values.get('password') != qcontext.get('confirm_password'):
            raise UserError(_("Passwords do not match; please retype them.")
                            
        )
        if len(national_id) <14 or len(national_id)>14 :
            raise UserError(_("please enter the correct number")
                            
        )
        if national_id[0] != '2' and national_id[0] != '3' :
            raise UserError(_("you are died can we catch photo with you")
                            
        )
        if int(national_id[1:3])>today or int(national_id[3:5])>12 or int(national_id[5:7])>31 :
            raise UserError(_("you are in the future")
                            
        )
        ph=values.get('national_id')
    ...
    def get_auth_signup_qcontext(self):
        ...
        SIGN_UP_REQUEST_PARAMS = {'db', 'login', 'debug', 'token', 'message', 'error', 'scope', 'mode',
                          ...
                          'password', 'confirm_password','national_id','city', 'country_id', 'lang', 'signup_email'}
